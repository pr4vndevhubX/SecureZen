...
const AISOCDashboard = () => {
        ...
        try {
            ...
            if (data.stats) {
                ...
                setStats(prev => ({
                    ...
                    minor: data.stats.severity_counts?.Medium || 0,
    ...
    return (
        <div className="min-h-screen bg-[#060914] text-gray-300 font-sans selection:bg-[#00d4ff]/30">
            {/* Header */}
            <header className="sticky top-0 z-50 bg-[#0a0e27] border-b border-[#1a1f3a] px-6 py-3 flex items-center justify-between shadow-2xl">
                <div className="flex items-center gap-8">
                    <div className="flex items-center gap-2 group cursor-pointer" onClick={() => setActiveTab('dashboard')}>
                        ...
                        <span className="text-2xl font-bold text-white tracking-tighter whitespace-nowrap">SecureZen</span>
                    </div>

                    <div className="hidden lg:flex items-center gap-6 border-l border-[#2d3748] pl-8">
                        <div className="flex flex-col items-center cursor-pointer hover:opacity-70 transition-opacity" onClick={() => navigateToEvents('All')}>
                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-none mb-1">Events Analyzed</span>
                            <span className="text-sm font-bold text-white">{stats.totalEvents}</span>
                        </div>
                        <div className="flex flex-col items-center cursor-pointer hover:opacity-70 transition-opacity" onClick={() => navigateToEvents('All')}>
                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-none mb-1">Threat Indicators</span>
                            <span className="text-sm font-bold text-white">{stats.threatScenarios}</span>
                        </div>
                        <div className="flex flex-col items-center border-l border-[#2d3748] pl-6 cursor-pointer hover:opacity-70 transition-opacity" onClick={() => navigateToEvents('High')}>
                            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest leading-none mb-1">Open Alerts</span>
                            <span className="text-sm font-bold text-[#00d4ff]">{stats.openAlerts}</span>
                        </div>
                    </div>
                </div>
            ...
            {/* Navigation Tabs */}
            <nav className="bg-[#0a0e27] border-b border-[#1a1f3a] px-6 sticky top-[61px] z-40 shadow-xl">
                <div className="flex gap-8">
                    {['dashboard', 'events', 'framework', 'intelligence'].map((tab) => (
                        <button
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`py-4 text-[11px] font-bold uppercase tracking-[0.2em] transition-all relative ${activeTab === tab ? 'text-[#00d4ff]' : 'text-gray-500 hover:text-white'}`}
                        >
                            {tab}
                            {activeTab === tab && (
                                <motion.div
                                    layoutId="activeTabUnderline"
                                    className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#00d4ff] shadow-[0_0_10px_#00d4ff]"
                                />
                            )}
                        </button>
                    ))}
            ...
            {/* Main Content */}
            <main className="p-6 max-w-[1720px] mx-auto">
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
                                {[
                                    ...
                                    { label: 'Major Alerts', count: stats.major, color: 'text-orange-500', bg: 'bg-orange-500/10', sev: 'High', icon: AlertTriangle },
                                    { label: 'Minor Alerts', count: stats.minor, color: 'text-yellow-500', bg: 'bg-yellow-500/10', sev: 'Medium', icon: AlertTriangle },
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                                ...
                                <div className="lg:col-span-4 bg-[#0a0e27] rounded-3xl overflow-hidden border border-[#1a1f3a] shadow-2xl h-[400px]">
                                    <ThreatFunnel
                                        ...
                                        onAlertsClick={() => navigateToEvents('High')}
                                    />
                                </div>
                                {/* Right: Trend Chart */}
                                    ...
                                    <div className="h-[320px]">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <PieChart>
                                                <Pie
                                                    data={[
                                                        { name: 'Minor', value: stats.minor || 0, fill: '#4daaf8', severity: 'Medium' },
                                                        { name: 'Major', value: stats.major || 0, fill: '#24b8ea', severity: 'High' },
