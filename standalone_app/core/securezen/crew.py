"""
IP Intelligence Crew - Professional CrewBase Implementation for Standalone App
...
# standalone_app/core/securezen/crew.py -> standalone_app
...
# Import tools (absolute imports within app_root context)
...
class IPIntelligenceCrew:
    ...
    def run_and_store(self, ioc: str):
        ...
        print(f"Starting CrewAI enrichment for IOC: {ioc}")
        try:
            from utils.database import ThreatDatabase
            db = ThreatDatabase()
            
            # Execute crew
            result = self.crew().kickoff(inputs={'ip_addresses': ioc})
...
if __name__ == "__main__":
    print("Building IP Intelligence Crew (Standalone)...\n")
    try:
